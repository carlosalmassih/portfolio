(() => {
  ...
  const url = value => { if(!value)return ''; try {const u=new URL(value,document.baseURI);return ['http:','https:','file:'].includes(u.protocol)?u.href:'';}catch{return '';}};
  ...
  document.querySelector('nav a[href="blog/"]')?.setAttribute('aria-current','page');
  if(page==='home')document.querySelector('nav a[href="blog/"]')?.removeAttribute('aria-current');
  ...
  if(pathParts.at(-1)==='index.html')pathParts.pop();
  ...
  root.replaceChildren();const back=make('a','text-link','← All stories');back.href='blog/';
  ...
  root.append(body);const foot=make('div','story-end');const work=make('a','text-link','Explore the photography ↗');work.href='portfolio/?media=photo';foot.append(work);root.append(foot);
})();
